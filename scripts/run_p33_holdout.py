"""Frozen P33 holdout을 P32-B candidate 변경 없이 한 번 실행한다."""
...
def main() -> None:
    ...
    parser.add_argument("--raw-output", type=Path, default=ROOT / "data/diagnostics/p33_holdout_raw.json")
    parser.add_argument("--execution-output", type=Path, default=ROOT / "evaluation/p33_holdout_execution.jsonl")
    args = parser.parse_args()
    if not args.execute:
        raise SystemExit("P33은 실제 HCX 비용이 발생합니다. --execute가 필요합니다.")

    manifest = json.loads(args.manifest.read_text(encoding="utf-8"))
    questions = manifest["questions"]
    denominators = manifest["denominators"]
    if len(questions) != denominators["total"]:
        raise SystemExit("P33 manifest total denominator가 질문 수와 일치하지 않습니다.")
    for behavior, expected_count in (("answerable", denominators["answerable"]), ("unsupported", denominators["unsupported"])):
        if sum(question["answerability"] == behavior for question in questions) != expected_count:
            raise SystemExit(f"P33 manifest {behavior} denominator가 일치하지 않습니다.")

    load_dotenv(ROOT / ".env")
    settings = GenerationSettings.from_env()
    if settings.generator_backend != "hcx" or settings.hcx_model != "HCX-007":
        raise SystemExit("P33에는 GENERATOR_BACKEND=hcx 및 HCX_MODEL=HCX-007이 필요합니다.")
    ...
    for question in questions:
        response = client.get(
            "/answer",
            params={"question_id": question["question_id"], "question": question["question"], "top_k": 10},
        )
        ...
        rows.append({
            ...
            "expected_policy_behavior": question["expected_policy_behavior"],
            "required_requirements": question["required_requirements"],
            "gold_answer_points": question["gold_answer_points"],
            "acceptable_equivalent_evidence": question["acceptable_equivalent_evidence"],
            "required_original_sources": question["required_original_sources"],
            ...
            "requirement_plan": trace.get("requirement_plan"),
            "evidence_sufficient": trace.get("evidence_sufficient"),
            "evidence_reason": trace.get("assessment_reason"),
            "missing_requirement_slots": trace.get("missing_requirement_slots", []),
            "generator_attempted": trace.get("generator_attempted", False),
            "generator_called": trace.get("generator_called", False),
            "generation_error": trace.get("generation_error"),
            "citation_valid": bool(trace.get("cited_chunk_ids")) and trace.get("generation_error") is None
            if trace.get("generator_called")
            else True,
    ...
    payload = {
        "experiment": "P33 fresh frozen holdout",
        "manifest_version": manifest["version"],
        "manifest_sha256": hashlib.sha256(args.manifest.read_bytes()).hexdigest(),
        "candidate": manifest["candidate"],
        "model": settings.hcx_model,
        "fixed_conditions": {
            "candidate_code": "unchanged",
