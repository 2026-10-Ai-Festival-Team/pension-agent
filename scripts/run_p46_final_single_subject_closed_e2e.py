"""Execute frozen P46 through the already-frozen P45 single-subject path."""
...
from scripts.run_p45_final_single_subject_closed_e2e import _output_row, _settings, _summary, PACING_SECONDS
...
HOLDOUT = ROOT / "question_bank/holdouts/p46_final_single_subject_closed_e2e.jsonl"
METADATA = ROOT / "question_bank/holdouts/p46_final_single_subject_closed_e2e_metadata.json"
OUTPUT = ROOT / "evaluation/p46_final_single_subject_closed_e2e.json"
CHECKPOINT = ROOT / "evaluation/.p46_final_single_subject_closed_e2e_checkpoint.jsonl"
...
def main() -> None:
    ...
    parser.add_argument("--execute", action="store_true", help="Required: P46 makes live selector and answer calls.")
    args = parser.parse_args()
    if not args.execute:
        parser.error("P46 is a frozen fresh holdout; pass --execute only after reviewing its manifest")
    ...
    if metadata.get("manifest_sha256") != _sha(rows) or not metadata.get("frozen_before_hcx"):
        raise RuntimeError("P46 manifest is not frozen and validated")
    settings = _settings()
    ...
    payload = {
        "experiment": "P46 Final Fresh Single-Subject Closed E2E",
