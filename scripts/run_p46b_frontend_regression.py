"""Recheck P46's three front-end failures with the frozen answer path disabled.

P46 is a development regression set after its fresh E2E execution.  This
script performs selector calls only: it neither calls answer generation nor
changes the candidate/browser route.
"""
...
OUTPUT = ROOT / "evaluation/p46b_frontend_regression.json"
CORPUS = ROOT / "data/parsed/chunks.jsonl"
INDEX = ROOT / "data/indexes/bm25/simple"
PACING_SECONDS = 6.0
TARGETS = {"P46-003", "P46-010", "P46-017"}
...
def _settings() -> GenerationSettings:
    ...
    if settings.generator_backend.lower() != "hcx" or settings.hcx_model.upper() != "HCX-007":
        raise RuntimeError("P46-B requires HCX-007 Native Structured Outputs")
    if not settings.hcx_api_key or not settings.hcx_base_url:
        raise RuntimeError("P46-B requires HCX_API_KEY and HCX_BASE_URL")
    return replace(settings, hcx_min_interval_seconds=PACING_SECONDS)


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--execute", action="store_true", help="Required because P46-B makes three live selector calls.")
    args = parser.parse_args()
    if not args.execute:
        parser.error("P46-B uses live selector calls; pass --execute")
    rows = [json.loads(line) for line in MANIFEST.read_text(encoding="utf-8").splitlines() if line.strip()]
    rows = [row for row in rows if row["id"] in TARGETS]
    if {row["id"] for row in rows} != TARGETS:
        raise RuntimeError("P46-B targets are missing from the P46 development manifest")
    ...
    for row in rows:
        ...
        outputs.append({
            ...
            "scope_correct": scoped.active_subject == row["expected_active_subject"],
            "requirement_exact": returned == expected,
            "gold_evidence_covered": evidence,
    ...
    OUTPUT.write_text(json.dumps({"experiment": "P46-B Front-end Regression", "summary": summary, "outputs": outputs}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
