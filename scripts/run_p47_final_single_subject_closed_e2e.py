"""Execute the frozen P47 fresh holdout through the P45-B single-subject path."""
...
OUTPUT = ROOT / "evaluation/p47_final_single_subject_closed_e2e.json"
CHECKPOINT = ROOT / "evaluation/.p47_final_single_subject_closed_e2e_checkpoint.jsonl"
...
def main() -> None:
    ...
    parser.add_argument("--execute", action="store_true", help="Required: P47 makes live selector and answer calls.")
    args = parser.parse_args()
    if not args.execute:
        parser.error("P47 is a frozen fresh holdout; pass --execute only after reviewing its manifest")
    ...
    if metadata.get("manifest_sha256") != _sha(rows) or not metadata.get("frozen_before_hcx"):
        raise RuntimeError("P47 manifest is not frozen and validated")
    settings = _settings()
