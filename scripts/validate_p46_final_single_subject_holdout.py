"""Validate and freeze P46's fresh single-subject Closed factual manifest."""
...
def main() -> None:
    rows = _rows(HOLDOUT)
    if len(rows) != 18 or {row["id"] for row in rows} != {f"P46-{i:03d}" for i in range(1, 19)}:
        raise RuntimeError("P46 requires 18 stable manually annotated rows")
    if any(row.get("annotation_source") != "manual" or row.get("auto_converted") for row in rows):
        raise RuntimeError("P46 requires manual non-auto-converted gold")
    if any(row.get("answerability") != "answerable" or row.get("expected_reference_behavior") != "resolved" for row in rows):
        raise RuntimeError("P46 is answerable, resolved single-subject Closed factual only")
    if any(not isinstance(row.get("expected_active_subject"), str) or not row["expected_active_subject"] for row in rows):
        raise RuntimeError("P46 requires one active subject per row")
    ...
    if len(normalized) != len(set(normalized)):
        raise RuntimeError("P46 contains internal normalized duplicates")
    ...
    if duplicates:
        raise RuntimeError(f"P46 normalized overlap: {duplicates}")
    metadata = {
