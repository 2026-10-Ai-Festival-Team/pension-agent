"""Validate and freeze P48's fresh single-subject Closed factual manifest."""
...
def main() -> None:
    rows = _rows(HOLDOUT)
    if len(rows) != 18 or {row["id"] for row in rows} != {f"P48-{index:03d}" for index in range(1, 19)}:
        raise RuntimeError("P48 requires 18 stable manually annotated rows")
    if any(row.get("annotation_source") != "manual" or row.get("auto_converted") for row in rows):
        raise RuntimeError("P48 requires manual non-auto-converted gold")
    if any(row.get("answerability") != "answerable" or row.get("expected_reference_behavior") != "resolved" for row in rows):
        raise RuntimeError("P48 is answerable, resolved single-subject Closed factual only")
    if any(not isinstance(row.get("expected_active_subject"), str) or not row["expected_active_subject"] for row in rows):
        raise RuntimeError("P48 requires one active subject per row")
    ...
    missing = sorted({chunk_id for row in rows for evidence in row["gold_evidence"].values() for chunk_id in evidence if chunk_id not in corpus_ids})
    if missing:
        raise RuntimeError(f"P48 gold chunks missing from corpus: {missing}")
    normalized = [normalize_pension_question(row["question"]) for row in rows]
    if len(normalized) != len(set(normalized)):
        raise RuntimeError("P48 contains internal normalized duplicates")
    ...
    if duplicates:
        raise RuntimeError(f"P48 normalized overlap: {duplicates}")
    metadata = {
        "experiment": "P48 Final Fresh Single-Subject Closed E2E", "question_count": 18,
        ...
        "execution_preparation_contract": "DirectRequirementE2EAgent._frontend_payload passes status, active_subject, selected_requirements, and scoped allowed_requirements to ScopedFrontendPreparationShadow.prepare",
        "manifest_sha256": hashlib.sha256(json.dumps(rows, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest(),
        "frozen_before_hcx": True, "hcx_calls": 0,
    }
